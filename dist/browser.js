(() => {
  ...
  var Client = class _Client {
    ...
    constructor(...options2) {
      ...
      for (const verb of this.#verbs) {
        this[verb] = async function(...options3) {
    ...
    fetch(req, options2) {
      ...
      if (!(typeof options2 === "object") || options2 instanceof String) {
        throw metroError("metro.client.fetch: Invalid options parameter " + metroURL + "client/fetch-invalid-options/", options2);
      }
    ...
    with(...options2) {
      return new _Client(this, ...options2);
    }
  };
  function client2(...options2) {
    return new Client(...options2);
  }
        ...
        switch (prop) {
          ...
          case "with":
            return function(...options3) {
              if (data) {
                options3.unshift({ body: data });
              }
              return request(target, ...options3);
            };
            break;
          case "data":
            return data;
    ...
    return new Proxy(r, {
      get(target, prop, receiver) {
        switch (prop) {
          ...
          case "with":
            return function(...options3) {
              return response(target, ...options3);
            };
            break;
          case "data":
            return data;
          ...
          switch (param) {
            ...
            default:
              ...
              if (typeof option[param] == "function") {
                ...
                throw metroError("metro.url: unsupported value for " + param + " " + metroURL + "url/unsupported-param-value/", options2[param]);
              }
              break;
          }
        }
    ...
    return new Proxy(params2, {
      get: (target, prop, receiver) => {
        switch (prop) {
          ...
          case "with":
            return function(...options3) {
              return formdata(target, ...options3);
            };
            break;
        }
        if (target[prop] instanceof Function) {
  ...
  function oauth2mw(options2) {
    ...
    assert(options2, {});
    const oauth2 = Object.assign({}, defaultOptions.oauth2_configuration, options2?.oauth2_configuration);
    options2 = Object.assign({}, defaultOptions, options2);
    options2.oauth2_configuration = oauth2;
    const store = tokenStore(options2.site);
    if (!options2.tokens) {
      options2.tokens = store.tokens;
    }
    if (!options2.state) {
      options2.state = store.state;
    }
    assert(options2, {
      oauth2_configuration: {
        client_id: Required(/.+/),
    ...
    for (let option in oauth2) {
      switch (option) {
        ...
        case "refresh_token":
          options2.tokens.set(option, oauth2[option]);
          break;
      }
    }
    return async function(req, next) {
      if (options2.force_authorization) {
    ...
    async function oauth2authorized(req, next) {
      ...
      let accessToken = options2.tokens.get("access_token");
      if (!accessToken) {
        try {
    ...
    function getTokensFromLocation() {
      if (typeof window !== "undefined" && window?.location) {
        ...
        if (params2) {
          ...
          let storedState = options2.state.get("metro/state");
          if (!state || state !== storedState) {
            return;
          }
          if (code) {
            options2.tokens.set("authorization_code", code);
          }
        }
      }
    }
    async function fetchAccessToken() {
      if (oauth2.grant_type === "authorization_code" && !options2.tokens.has("authorization_code")) {
        let authReqURL = await getAuthorizationCodeURL();
        if (!options2.authorize_callback || typeof options2.authorize_callback !== "function") {
        ...
        let token = await options2.authorize_callback(authReqURL);
        if (token) {
          options2.tokens.set("authorization_code", token);
        } else {
          return false;
        }
      }
      let tokenReq = getAccessTokenRequest();
      let response2 = await options2.client.post(tokenReq);
      if (!response2.ok) {
      ...
      options2.tokens.set("access_token", {
      ...
      if (data.refresh_token) {
        ...
        options2.tokens.set("refresh_token", token);
      }
      return data;
    }
    async function fetchRefreshToken() {
      let refreshTokenReq = getAccessTokenRequest("refresh_token");
      let response2 = await options2.client.post(refreshTokenReq);
      if (!response2.ok) {
    ...
    async function getAuthorizationCodeURL() {
      ...
      options2.state.set(search.state);
      if (oauth2.code_verifier) {
        options2.tokens.set("code_verifier", oauth2.code_verifier);
    ...
    function getAccessTokenRequest(grant_type = null) {
      ...
      const code_verifier = options2.tokens.get("code_verifier");
      if (code_verifier) {
      ...
      switch (oauth2.grant_type) {
        case "authorization_code":
          ...
          params2.code = options2.tokens.get("authorization_code");
          break;
    ...
    return async (req, next) => {
      ...
      if (["POST", "PUT", "PATCH", "QUERY"].includes(req.method)) {
        if (req.data && typeof req.data == "object" && !(req.data instanceof ReadableStream)) {
          ...
          req = req.with({
            body: JSON.stringify(req.data, options2.replacer, options2.space)
          });
        }
      }
      ...
      if (isJSON(res.headers.get("content-type"))) {
        ...
        try {
          let json = JSON.parse(body, options2.reviver);
          return res.with({
            body: json
          });
        } catch (e) {
  ...
  function thrower(options2) {
    return async (req, next) => {
      let res = await next(req);
      if (!res.ok) {
        if (options2 && typeof options2[res.status] == "function") {
          res = options2[res.status].apply(res, req);
        } else {
  ...
  async function generateKeyPair(alg, options2) {
    let algorithm;
    ...
    switch (alg) {
      case "PS256":
        algorithm = {
          name: "RSA-PSS",
          hash: "SHA-256",
          modulusLength: options2?.modulusLength ?? 2048,
    ...
    return crypto.subtle.generateKey(algorithm, options2?.extractable ?? false, ["sign", "verify"]);
  }
  ...
  function dpopmw(options2) {
    ...
    return async (req, next) => {
      ...
      let keyInfo = await keys.get(options2.site);
      if (!keyInfo) {
        let keyPair = await generateKeyPair("ES256");
        keyInfo = { domain: options2.site, keyPair };
        await keys.set(keyInfo);
      }
      const url3 = everything_default.url(req.url);
      if (req.url.startsWith(options2.authorization_endpoint)) {
      ...
      } else if (req.url.startsWith(options2.token_endpoint)) {
  ...
  var MustHave = (...options2) => (value, root) => {
    if (options2.filter((o) => root.hasOwnKey(o)).length > 0) {
      return false;
    }
    return error("root data must have all of", root, options2);
  };
  var MustInclude = (...options2) => (value) => {
    if (Array.isArray(value) && options2.filter((o) => !value.includes(o)).length == 0) {
      return false;
    } else {
      return error("data must be an array which includes", value, options2);
    }
  };
  var validJWA = [
    "HS256",
  ...
  async function oidcDiscovery(options2 = {}) {
    assert(options2, {
    ...
    const openid_provider_metadata = {
      issuer: Required(allOf(options2.issuer, MustUseHTTPS)),
      authorization_endpoint: Required(validURL),
      token_endpoint: Required(validURL),
      userinfo_endpoint: Recommended(validURL),
      // todo: test for https protocol
      jwks_uri: Required(validURL),
      registration_endpoint: options2.requireDynamicRegistration ? Required(validURL) : Recommended(validURL),
      scopes_supported: Recommended(MustInclude("openid")),
      response_types_supported: options2.requireDynamicRegistration ? Required(MustInclude("code", "id_token", "id_token token")) : Required([]),
      response_modes_supported: Optional([]),
      grant_types_supported: options2.requireDynamicRegistration ? Optional(MustInclude("authorization_code")) : Optional([]),
    ...
    const configURL = everything_default.url(options2.issuer, ".well-known/openid-configuration");
    const response2 = await options2.client.get(
    ...
    assert(openid_config.issuer, options2.issuer);
    return openid_config;
  }

  // node_modules/@muze-nl/metro-oidc/src/oidc.register.mjs
  async function register(options2) {
    ...
    options2 = Object.assign({}, defaultOptions, options2);
    let response2 = await options2.client.post(options2.registration_endpoint, {
      body: options2.client_info
    });
    ...
    options2.client_info = Object.assign(options2.client_info, info);
    return options2.client_info;
  }
  ...
  function oidcmw(options2 = {}) {
    ...
    options2 = Object.assign({}, defaultOptions, options2);
    assert(options2, {
      client: Required(instanceOf(client2().constructor)),
    ...
    if (!options2.store) {
      options2.store = oidcStore(options2.issuer);
    }
    if (!options2.openid_configuration && options2.store.has("openid_configuration")) {
      options2.openid_configuration = options2.store.get("openid_configuration");
    }
    if (!options2.client_info.client_id && options2.store.has("client_info")) {
      options2.client_info = options2.store.get("client_info");
    }
    return async (req, next) => {
      let res;
      if (!options2.force_authorization) {
      ...
      if (!options2.openid_configuration) {
        options2.openid_configuration = await oidcDiscovery({
          issuer: options2.issuer
        });
        options2.store.set("openid_configuration", options2.openid_configuration);
      }
      if (!options2.client_info?.client_id) {
        assert(options2.client_info?.client_name, Required());
        if (!options2.openid_configuration.registration_endpoint) {
          throw metroError("metro.oidcmw: Error: issuer " + options2.issuer + " does not support dynamic client registration, but you haven't specified a client_id");
        }
        options2.client_info = await register({
          registration_endpoint: options2.openid_configuration.registration_endpoint,
          client_info: options2.client_info
        });
        options2.store.set("client_info", options2.client_info);
      }
      const scope = options2.scope || "openid";
      const oauth2Options = Object.assign(
        {
          site: options2.issuer,
          client: options2.client,
          force_authorization: true,
          authorize_callback: options2.authorize_callback,
          oauth2_configuration: {
            client_id: options2.client_info.client_id,
            client_secret: options2.client_info.client_secret,
            grant_type: "authorization_code",
            response_type: "code",
            response_mode: "query",
            authorization_endpoint: options2.openid_configuration.authorization_endpoint,
            token_endpoint: options2.openid_configuration.token_endpoint,
            scope,
            //FIXME: should only use scopes supported by server
            redirect_uri: options2.client_info.redirect_uris[0]
          }
        }
        //...
      );
      const storeIdToken = async (req2, next2) => {
        ...
        if (contentType?.startsWith("application/json")) {
          ...
          if (id_token) {
            options2.store.set("id_token", id_token);
          }
        }
        return res2;
      };
      let oauth2client = options2.client.with(options2.issuer).with(storeIdToken);
      if (options2.use_dpop) {
        const dpopOptions = {
          site: options2.issuer,
          authorization_endpoint: options2.openid_configuration.authorization_endpoint,
          token_endpoint: options2.openid_configuration.token_endpoint,
          dpop_signing_alg_values_supported: options2.openid_configuration.dpop_signing_alg_values_supported
        };
...
`;
  ...
  var require_readable = __commonJS({
    "node_modules/readable-stream/lib/internal/streams/readable.js"(exports, module) {
      ...
      function ReadableState(options2, stream, isDuplex) {
        ...
        if (options2 && options2.objectMode)
        ...
        this.flowing = null;
        this[kPaused] = null;
        if (options2 && options2.emitClose === false)
      ...
      async function* createAsyncIterator(stream, options2) {
        ...
        try {
          ...
          if ((error2 || (options2 === null || options2 === void 0 ? void 0 : options2.destroyOnReturn) !== false) && (error2 === void 0 || stream._readableState.autoDestroy)) {
  ...
  var require_transform = __commonJS({
    "node_modules/readable-stream/lib/internal/streams/transform.js"(exports, module) {
      ...
      function Transform3(options2) {
        if (!(this instanceof Transform3))
          return new Transform3(options2);
  ...
  var require_operators = __commonJS({
    "node_modules/readable-stream/lib/internal/streams/operators.js"(exports, module) {
      ...
      async function find(fn, options2) {
        for await (const result2 of filter.call(this, fn, options2)) {
          return result2;
        }
        return void 0;
      }
  ...
  function oldm(options2) {
    return new Context(options2);
  }
  ...
  var Context = class {
    constructor(options2) {
      this.prefixes = { ...prefixes, ...options2?.prefixes };
      if (!this.prefixes["xsd"]) {
        this.prefixes["xsd"] = "http://www.w3.org/2001/XMLSchema#";
      }
      this.parser = options2?.parser;
    ...
    parse(input, url3, type) {
      ...
      if (prefixes3) {
        for (let prefix2 in prefixes3) {
          let prefixURL = new URL(prefixes3[prefix2], url3).href;
  ...
  var N3Lexer = class {
    constructor(options2) {
      ...
      this._endOfFile = /^(?:#[^\n\r]*)?$/;
      options2 = options2 || {};
      if (this._lineMode = !!options2.lineMode) {
        this._n3Mode = false;
        ...
        this._n3Mode = options2.n3 !== false;
      }
      this.comments = !!options2.comments;
  ...
  var N3Parser = class {
    constructor(options2) {
      ...
      this._graph = null;
      options2 = options2 || {};
      this._setBase(options2.baseIRI);
      options2.factory && initDataFactory(this, options2.factory);
      const format = typeof options2.format === "string" ? options2.format.match(/\w*$/)[0].toLowerCase() : "", isTurtle = /turtle/.test(format), isTriG = /trig/.test(format), isNTriples = /triple/.test(format), isNQuads = /quad/.test(format), isN3 = this._n3Mode = /n3/.test(format), isLineMode = isNTriples || isNQuads;
      ...
      this._blankNodePrefix = typeof options2.blankNodePrefix !== "string" ? "" : options2.blankNodePrefix.replace(/^(?!_:)/, "_:");
      this._lexer = options2.lexer || new N3Lexer({ lineMode: isLineMode, n3: isN3 });
      this._explicitQuantifiers = !!options2.explicitQuantifiers;
    }
  ...
  var N3Writer = class {
    constructor(outputStream, options2) {
      this._prefixRegex = /$0^/;
      if (outputStream && typeof outputStream.write !== "function")
        options2 = outputStream, outputStream = null;
      options2 = options2 || {};
      this._lists = options2.lists;
      if (!outputStream) {
        let output = "";
        this._outputStream = {
        ...
        this._endStream = options2.end === void 0 ? true : !!options2.end;
      }
      this._subject = null;
      if (!/triple|quad/i.test(options2.format)) {
        this._lineMode = false;
        this._graph = DEFAULTGRAPH2;
        this._prefixIRIs = /* @__PURE__ */ Object.create(null);
        options2.prefixes && this.addPrefixes(options2.prefixes);
        if (options2.baseIRI) {
          this._baseMatcher = new RegExp(`^${escapeRegex(options2.baseIRI)}${options2.baseIRI.endsWith("/") ? "" : "[#?]"}`);
          this._baseLength = options2.baseIRI.length;
        }
      } else {
        this._lineMode = true;
...
`);
  ...
  var N3EntityIndex = class {
    constructor(options2 = {}) {
      ...
      this._factory = options2.factory || N3DataFactory_default;
    }
    _termFromId(id2) {
      if (id2[0] === ".") {
  ...
  var N3Store = class _N3Store {
    constructor(quads, options2) {
      this._size = 0;
      this._graphs = /* @__PURE__ */ Object.create(null);
      if (!options2 && quads && !quads[0])
        options2 = quads, quads = null;
      options2 = options2 || {};
      this._factory = options2.factory || N3DataFactory_default;
      this._entityIndex = options2.entityIndex || new N3EntityIndex({ factory: this._factory });
  ...
  var DatasetCoreAndReadableStream = class _DatasetCoreAndReadableStream extends import_readable_stream.Readable {
    constructor(n3Store, subject, predicate, object, graph, options2) {
      super({ objectMode: true });
      Object.assign(this, { n3Store, subject, predicate, object, graph, options: options2 });
    }
    get filtered() {
      if (!this._filtered) {
  ...
  var N3StreamParser = class extends import_readable_stream2.Transform {
    constructor(options2) {
      ...
      const parser = new N3Parser(options2);
      let onData, onEnd;
      const callbacks = {
  ...
  function one(selectFn, whichOne = "last") {
    return (data, key, context) => {
      let result2 = selectFn(data, key, context);
  ...
  var HttpAdapter = class _HttpAdapter {
    ...
    async #fetch(path, options2) {
      return fetch(this.#getUrl(path), options2).catch((e) => {
        if (!this.#exceptionHandler || !this.#exceptionHandler(url, options2, e)) {
          throw e;
        }
      });
    }
  ...
  var solidClient = class {
    ...
    constructor(options2) {
      let defaultOptions = {
        ...
        oidc: {
          client_info: {
            client_id: url2(window.location).authority
          }
        }
      };
      this.options = Object.assign({}, defaultOptions, options2);
      this.#client = client2(options2).with(oidcmw(options2.oidc));
    }
    login(webId2, force = false) {
      const session = new solidSession(this.#client, webId2, this.options);
      session.login(force);
      return session;
    }
  };
  var client_default = solidClient;
  function oldmmw(prefixes3, context) {
    return async (req, next) => {
  ...
  var solidSession = class {
    #client;
    #options;
    context;
    profile;
    // FIXME: constructor overloaded, split into two parts?
    constructor(client3, webId2, options2) {
      this.#options = options2;
      this.context = oldm2.context({
        prefixes: options2.prefixes,
        parser: oldm2.n3Parser,
        writer: oldm2.n3Writer
      });
      this.#client = client3.with(oldmmw(options2.prefixes, this.context));
    }
    ...
    async logout() {
      if (options.openid_configuration.end_session_endpoint) {
        let response2 = await this.#client.get(options.openid_configuration.end_session_endpoint, {
    ...
    pod(podURI = null, context) {
      if (!podURI) {
        podURI = from(this.profile).select(one(_.pim$storage));
      }
      if (!podURI) {
        throw new Error("No pim:storage found in users profile");
      }
      return browser_default.fs(new Pod(this.#client, context, podURI));
    }
    // TODO: load resource and return it as oldm Graph
    get() {
  ...
  var Pod = class _Pod {
    ...
    #path;
    #context;
    constructor(client3, context, URI, path = "/") {
      this.#client = client3;
      this.#baseUrl = URI;
      this.#path = path;
      this.#context = context;
    ...
    cd(path) {
      ...
      return new _Pod(this.#client, this.#baseUrl, path);
    }
    async write(path, contents, metadata = null) {
    }
    writeStream(path, writer, metadata = null) {
    ...
    async read(path) {
      path = this.#getPath(path);
      let response2 = await this.#client.get(url2(this.#baseUrl, path));
      return {
        type: response2.headers.get("Content-Type"),
        name: browser_default.Path.filename(path),
        http: {
          headers: response2.headers,
          status: response2.status,
          url: response2.url
        },
        contents: response2.data
        //parsed by oldmmw
      };
    }
    readStream(path, reader) {
    ...
    async exists(path) {
      path = this.#getPath(path);
      this.#client.head(url2(this.#baseUrl, path));
    }
    async delete(path) {
      path = this.#getPath(path);
      this.#client.delete(url2(this.#baseUrl, path));
    }
    async list(path = null) {
      path = this.#getPath(path);
      const result2 = await this.read(path);
      return from(result2.contents).where({
        a: _.ldp$Resource
      }).select({
        filename: (o) => browser_default.Path.filename(url2(o.id)),
        path: (o) => url2(o.id).pathname,
        name: (o) => browser_default.Path.filename(url2(o.id))
    ...
    #getPath(path) {
      ...
      if (!browser_default.Path.isPath(path)) {
        throw new TypeError(path + " is not a valid path");
      }
      return path;
    }
